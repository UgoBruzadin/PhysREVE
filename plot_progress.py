...
def plot_progress(records: list[dict], save: bool = False, show: bool = True):
    ...
    ax.plot(x, baseline, color=GREY,  marker="o", lw=2, ms=7, label="Baseline (random init)")
    ax.plot(x, physreve, color=BLUE,  marker="o", lw=2, ms=7, label="PhysREVE (physics pretrained)")
    ax.axhline(CHANCE, ls="--", color=RED, lw=1.2, alpha=0.6, label="Chance (25%)")

    # Shade the gap between the two lines
    ax.fill_between(x, baseline, physreve,
                    where=[p >= b for p, b in zip(physreve, baseline)],
                    alpha=0.12, color=GREEN, label="PhysREVE ahead")
    ax.fill_between(x, baseline, physreve,
                    where=[p < b for p, b in zip(physreve, baseline)],
                    alpha=0.12, color=RED,   label="Baseline ahead")

    # Annotate each point with its value
    for xi, (b, p) in enumerate(zip(baseline, physreve)):
        ax.annotate(f"{p:.1f}%", (xi, p), textcoords="offset points",
                    xytext=(0, 8), ha="center", fontsize=9, color=BLUE, fontweight="bold")
        ax.annotate(f"{b:.1f}%", (xi, b), textcoords="offset points",
                    xytext=(0, -14), ha="center", fontsize=9, color=GREY)

    ax.set_xticks(x)
    ax.set_xticklabels(ids, fontsize=10)
    ax.set_ylabel("Test Accuracy (%)", fontsize=10)
    ax.set_title("Model Accuracy per Experiment", fontsize=11)
    ax.set_ylim(0, 75)
    ...
    ax2.set_ylabel("PhysREVE − Baseline (%)", fontsize=10)
    ax2.set_title("Improvement Over Baseline", fontsize=11)
    ...
    table_data = [[r["exp_id"], r["description"][:55] + ("…" if len(r["description"]) > 55 else "")]
                  for r in records]
    table = ax3.table(
        cellText=table_data,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save",    action="store_true", help="Save figure to experiments/progress.png")
    parser.add_argument("--no-show", action="store_true", help="Don't call plt.show() (headless)")
